...
Core detection algorithm (Laplacian V1):
  ...
  - Fix: compute statistics/threshold in float space (no premature 8-bit clipping).
  - Follow-up (per your feedback that blur was washing out real defect detail and
    noise was still getting through): replaced the single global mean/std threshold
    with a LOCAL sliding-window mean/std (same boxFilter Var=E[X^2]-E[X]^2 technique
    as the dust_inspector app's proven local Z-score), computed directly on the
    Laplacian response with no pre-blur. Threshold now adapts per-neighborhood instead
    of to the whole frame, so it handles uneven illumination and suppresses
    noise-driven false positives without smoothing away small real defects. Verified
    empirically across several window sizes/noise levels -- a window comfortably
    bigger than the defects themselves (default 51px) was needed, since a too-small
    window lets a defect's own huge Laplacian response skew the very statistics being
    used to judge it. New "Local Window (px)" control replaces the old "Noise Blur".
...
# =============================================================================
def detect_laplacian_v1(img, k_multiplier=3.0, ksize=3, window=51):
    if len(img.shape) == 3:
    ...
    # No pre-blur here on purpose: blurring smooths away small real defects along with
    # the noise (this was tried and visibly cost detail/dropped genuine defects).
    laplacian = cv2.Laplacian(gray, cv2.CV_64F, ksize=ksize)
    lap_abs = np.abs(laplacian)

    # LOCAL (sliding-window) statistics instead of one global mean/std for the whole
    # frame/ROI. A single global threshold bakes in whatever noise level happens to
    # dominate the WHOLE image: a noisy frame inflates it (misses real defects
    # everywhere), a quiet frame makes it too sensitive everywhere (including in
    # genuinely noisier patches, e.g. uneven lighting). Computing mean/std in a window
    # around each pixel instead means a pixel only fires if it stands out from its own
    # neighborhood, not from the image as a whole -- this also makes it robust to
    # gradients/uneven illumination across the frame.
    #
    # mean/std are computed via cv2.boxFilter using the standard
    # Var[X] = E[X^2] - E[X]^2 trick, which is what makes a per-pixel sliding window
    # practical (a naive per-pixel loop would be far too slow). The window must be
    # comfortably bigger than the defects you're looking for -- too small and a
    # defect's own (huge) Laplacian response starts dominating the very statistics
    # being used to judge it, which raises its own local threshold and can hide it
    # (verified empirically: an 11-21px window intermittently missed defects that a
    # 51px window caught cleanly, for the same defects/noise).
    ...
    binary_mask = (lap_abs > threshold_local).astype(np.uint8) * 255
...
class VisionInspectionApp:
    ...
    def _build_camera_controls(self):
        ...
        self.cam_backend_cb.bind("<<ComboboxSelected>>", lambda e: self.refresh_cameras())
        ...
        ttk.Label(cam_frame, text="Detected Camera:").grid(row=2, column=0, sticky="w", pady=4)
        self.cam_device_cb = ttk.Combobox(cam_frame, values=[], state="readonly", width=28)
        self.cam_device_cb.grid(row=2, column=1, columnspan=2, sticky="ew", padx=5, pady=4)

        ttk.Button(cam_frame, text="Refresh Cameras", command=self.refresh_cameras).grid(
        ...
        self.btn_cam_toggle.grid(row=4, column=0, columnspan=3, sticky="ew", pady=4)

        self.cam_devices = []
        self.refresh_cameras()

    def refresh_cameras(self):
        """Populate the 'Detected Camera' dropdown with whatever is actually connected
        right now for the currently selected Camera Type."""
        self.cam_devices = []
            ...
            if PYLON_AVAILABLE:
                try:
                    ...
                    for d in tl_factory.EnumerateDevices():
                        label = f"{d.GetFriendlyName()} (SN: {d.GetSerialNumber()})"
                        self.cam_devices.append({'label': label, 'backend': 'basler', 'value': d.GetSerialNumber()})
                except Exception as e:
                    messagebox.showerror("Basler Enumeration Error", str(e))
        else:
            # Probe a small range of OS camera indices for OpenCV-visible (UVC) cameras.
            for i in range(5):
                cap = cv2.VideoCapture(i)
                if cap.isOpened():
                    self.cam_devices.append({'label': f"OpenCV Camera Index {i}", 'backend': 'opencv', 'value': i})
                cap.release()

        labels = [d['label'] for d in self.cam_devices]
        self.cam_device_cb['values'] = labels
        if labels:
            self.cam_device_cb.current(0)
        else:
            self.cam_device_cb.set('')
    ...
    def _build_algorithm_controls(self):
        ...
        ttk.Label(algo_frame, text="Local Window (px):").grid(row=6, column=0, sticky="w", pady=4)
        self.window_cb = ttk.Combobox(algo_frame, values=["21", "31", "51", "75", "101"], width=5, state="readonly")
        self.window_cb.set("51")
        self.window_cb.grid(row=6, column=1, sticky="w", padx=5, pady=4)
    ...
    def toggle_camera(self):
        if self.cam and self.cam.started:
            ...
            idx = self.cam_device_cb.current()
            if idx < 0 or idx >= len(self.cam_devices):
                messagebox.showwarning("No Camera Selected", "Click 'Refresh Cameras' and pick one from the list first.")
                return
            device = self.cam_devices[idx]
            try:
                if device['backend'] == 'basler':
                    self.cam = BaslerCameraStream().start(serial_filter=device['value'])
                else:
                    self.cam = CameraStream(device['value']).start()
